        ...
        public static string[] TableNames(SE_CONNECTION connection)
        {
            try
            {
                unsafe
                {
                    ...
                    if (Wrapper92.SE_registration_get_info_list(connection, ref ptr, ref num) != 0) return null;
                    IntPtr* reginfo = (System.IntPtr*)ptr.pointer[0];

                    StringBuilder sb = new StringBuilder();
                    for (int i = 0; i < num; i++)
                    {
                        Int32 result = Wrapper92.SE_reginfo_has_layer(reginfo[i]);
                        if (result != 0)
                        {
                            byte[] buffer = new byte[CONST.SE_QUALIFIED_TABLE_NAME];
                            Wrapper92.SE_reginfo_get_table_name(reginfo[i], buffer);
                            string table = System.Text.Encoding.ASCII.GetString(buffer).Replace("\0", "");
                            sb.Append(table);

                            /*
                            buffer = new byte[CONST.SE_MAX_DESCRIPTION_LEN];
                            Wrapper92.SE_reginfo_get_description(reginfo[i], buffer);
                            string descr = System.Text.Encoding.ASCII.GetString(buffer).Replace("\0", "");
                            sb.Append(" ("+descr+")" + ";");
                             * */
                        }
                        Wrapper92.SE_reginfo_free(reginfo[i]);
                    }
        ...
        public static void GetLayerInfos(SE_CONNECTION connection, GetLayerInfosCallBack call)
        {
            ...
            try
            {
                unsafe
                {
                    ...
                    //System.Int32 status = Wrapper92.SE_layer_get_info_list(connection,ref ptr, ref num);
                    //if (status != 0)
                    //{
                    //    return;
                    //}
                    ...
                    //    call(layerInfo[i]);
                    //}
                    //if (num > 0) Wrapper92.SE_layer_free_info_list(num, ptr.pointer[0]);
                }
            }
            catch (Exception ex)
            {

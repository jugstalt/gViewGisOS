    ...
    public class SdeDataset : DatasetMetadata, IFeatureDataset, IDisposable, IDataset2, IPersistable, IPlugInDependencies
    {
        ...
        */
        ...
        internal SE_COORDREF GetSeCoordRef(ArcSdeConnection connection, string table, string spatialColumnName, ref SE_ENVELOPE envelope)
        {
            ...
            {
                if (layerInfo.handle != 0) Wrapper92.SE_layerinfo_free(layerInfo);
    
                if (_err_no != 0)
                {
                    if (coordRef.handle != 0) Wrapper92.SE_coordref_free(coordRef);
            ...
            get
            {
                if (_elements.Count == 0)
                {
                    ...
                    using (ArcSdeConnection conn = new ArcSdeConnection(_connStr))
                    {
                        ...
                        System.Int32 status = Wrapper92.SE_layer_get_info_list(conn.SeConnection, ref ptr, ref num);
                        if (status != 0)
                        {
                            return _elements;
                        }
                    ...
                    if (num > 0) Wrapper92.SE_layer_free_info_list(num, ptr);
                }
            ...
            get
            {
                ...
                try
                {
                    ...
                    using (ArcSdeConnection connection = new ArcSdeConnection(_connStr))
                    {
                        if (connection.Open())
                        {
                            if (Wrapper92.SE_layerinfo_create(new SE_COORDREF(), ref layerInfo) != 0) return null;
                            if (Wrapper92.SE_layer_get_info(connection.SeConnection, title, "", layerInfo) != 0) return null;

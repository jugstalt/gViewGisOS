...
namespace gView.Framework.system
{
	...
	public class PlugInManager
	{
		...
		static public string compFileName 
		{
			get 
			{
                if (!String.IsNullOrEmpty(SystemVariables.CustomMyCommonApplicationData))
                    return SystemVariables.CustomMyCommonApplicationData + @"\gViewGisOS_plugins.xml";

                string ret = "";
				try 
				{
					RegistryKey key=Registry.LocalMachine.OpenSubKey(@"Software\gViewGisOS",false);
					ret=key.GetValue("comp").ToString();
					key.Close();    
                ...
                if (ret == "")
                {
                    ret = SystemVariables.MyCommonApplicationData + @"\gViewGisOS_plugins.xml";
                }
                return ret;
			}
			set 

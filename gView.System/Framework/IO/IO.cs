...
namespace gView.Framework.IO
{
	...
	public class XmlStream : IPersistStream
	{
        ...
        private NumberFormatInfo _nhi = System.Globalization.CultureInfo.InvariantCulture.NumberFormat;
        private bool _performEncryption = true;

		public XmlStream(string rootname) 
		{
			_doc=new XmlDocument();
            _nhi = System.Globalization.CultureInfo.CurrentCulture.NumberFormat;
            
			if(rootname=="") rootname="root";
			
			_parent=new XmlNodePlus(this.CreateNode(rootname));
            _parent.Culture = System.Globalization.CultureInfo.InvariantCulture;
		...
		public object Load(string key,object defValue,object objectInstance)
		{
			...
			try 
			{
                ...
                {
                    Type type = System.Type.GetType(xmlnode.Attributes["type"].Value, false, true);
    ...
    public class PersistableClass : Component, ICustomTypeDescriptor,IPersistable
    {
                        ...
                        if (obj is XmlStreamObject)
                        {
                            ...
                            if (obj is XmlStreamOption)
                            {
                                ...
                                foreach (XmlNode optionNode in node.SelectNodes("Option[@type]"))
                                {
                                    ...
                                    Type optionType = System.Type.GetType(optionNode.Attributes["type"].Value, false, true);
                                ...
                                {
                                    Type valueType = System.Type.GetType(valueNode.Attributes["type"].Value, false, true);
        ...
        {
            public static Type FromArray(string enumName, object[] array)
            {
                ...
                myEnumBuilder = myModuleBuilder.DefineEnum("MyNamespace."+enumName, TypeAttributes.Public, typeof(System.Int32));

                int i = 0;
                foreach (object obj in array)
                {
                    myEnumBuilder.DefineLiteral(obj.ToString(), i++);
                }
	...
	public class ConfigTextStream 
	{
		...
		private System.IO.StreamReader tr=null;
		private System.IO.StreamWriter tw=null;
		...
		public ConfigTextStream(string name) 
		{
			...
			try 
			{
				...
				if(fi.Exists) tr=new System.IO.StreamReader(_path);
			} 
			catch 
			{
				Close();
		...
		public ConfigTextStream(string name,bool write,bool append) 
		{
			try 
			{
				...
				{
					tw=new System.IO.StreamWriter(_path,append); 
				}
			}
            catch/*(Exception ex)*/ 
			{
	...
	internal class XmlNodePlus 
	{
        ...
        private NumberFormatInfo _nhi = System.Globalization.CultureInfo.CurrentCulture.NumberFormat;

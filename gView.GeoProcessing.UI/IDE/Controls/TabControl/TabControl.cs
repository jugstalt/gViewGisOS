...
namespace IDE.Controls
{
    ...
    [Designer(typeof(IDE.Controls.TabControlDesigner))]
    internal class TabControl : UserControl
    {
        // Enumeration of appearance styles
        internal enum VisualAppearance
        {
            MultiDocument = 0,
            MultiForm = 1,
            MultiBox = 2
        ...
        internal delegate void DoubleClickTabHandler(TabControl sender, TabPage page);
        ...
        private ContextMenuStrip _headerContextMenuStrip = null;
        private int _contextTabPage = -1;

        static TabControl()
        {
            // Create a strip of images by loading an embedded bitmap resource
            _internalImages = ResourceHelper.LoadBitmapStrip(Type.GetType("IDE.Controls.TabControl"),
                                                             "PostInstallation.IDE.Resources.ImagesTabControl.bmp",
        ...
        {
            set { _headerContextMenuStrip = value; }
            get { return _headerContextMenuStrip; }
        }
        public int ContextTabPage
        {
            get { return _contextTabPage; }
        }
        ...
        protected virtual void DrawIDETabBorder(TabPage page, Graphics g, Rectangle rectPage)
        {
                      ...
                      darkdark = new Pen(_backDarkDark))
            {
                if (page.Selected)
                {
                    ...
                    using (SolidBrush pageAreaBrush = new SolidBrush(Color.FromArgb(234,233,221)))
                    ...
                    if (_positionAtTop)
                    {
                        ...
                        // Draw the top border
                        g.DrawLine(darkdark, rectPage.Left + 3, rectPage.Top, rectPage.Right - 3, rectPage.Top);

                        // Draw the right border
                        g.DrawLine(darkdark, rectPage.Right - 2, rectPage.Top, rectPage.Right, rectPage.Top + 2);
                        g.DrawLine(darkdark, rectPage.Right, rectPage.Top+2, rectPage.Right, rectPage.Bottom);
                    }
                    else
                    {
                        // Draw the left border
                        ...
                        // Draw the bottom border
                        g.DrawLine(darkdark, rectPage.Left + 3, rectPage.Bottom, rectPage.Right-3, rectPage.Bottom); 
        ...
        internal void ExternalMouseTest(IntPtr hWnd, Point mousePos)
        {
            if (!(ControlMouseTest(hWnd, mousePos, _leftArrow) ||
                ...
                InternalMouseDown(mousePos,MouseButtons.Left);
        }
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            InternalMouseDown(new Point(e.X, e.Y),e.Button);

            base.OnMouseDown(e);
        }
        
        protected virtual void InternalMouseDown(Point mousePos,MouseButtons button)
        {
            _contextTabPage = -1;
            ...
            for(int i=0; i<_tabPages.Count; i++)
            {
                ...
                if (rect.Contains(mousePos))
                {
                    ...
                    {
                        if (_tabPages[i].ContextMenuStrip != null)
                        {
                            _contextTabPage = i;
                            _tabPages[i].ContextMenuStrip.Show(this,mousePos, ToolStripDropDownDirection.Default);
                            return;
                        }
                    }
                }
            }
            if (button == MouseButtons.Right && _headerContextMenuStrip!=null)
            {
                HeaderContextMenuStrip.Show(this, mousePos, ToolStripDropDownDirection.Default);
            }
        }		
        ...
        private void InitializeComponent()
        {
            ...
            this.Size = new System.Drawing.Size(192, 282);

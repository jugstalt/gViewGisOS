...
namespace gView.Framework.Geometry
{
    ...
    public sealed class Envelope : IEnvelope, IGeometry
    {
        ...
        public IPolygon ToPolygon(int accuracy)
        {
    ...
    public sealed class GeometricTransformer : gView.Framework.Geometry.IGeometricTransformer, IDisposable
    {
...
#if(DEBUG)
                        LogSpatialReference(true, value);
#endif
...
#if(DEBUG)
                        LogSpatialReference(false, value);
#endif
                        //if (ParametersContain(value, "+nadgrids=@null"))
            ...
            if (geometry is PointCollection)
            {
                ...
                lock (LockThis1)
                {
                    try
                    {
                        ...
                        {
#if(DEBUG)
                            LogCoords(xPtr, yPtr, pointCount);
#endif
                            //if (preTo > 0)
                            //{
...
#else
        bool _log = false;
        string _logfile = @"C:\temp\geo_transformer.log";
        ...
        private void LogSpatialReference(bool from, ISpatialReference sRef)
        {
            if (_log == false)
                return;

            if (from)
                Log("From (" + _fromID + "): " + allParametersString(sRef));
            else
                Log("To (" + _toID + "): " + allParametersString(sRef));
        }

        private void LogCoords(IntPtr xPtr, IntPtr yPtr, int pointCoint)
        {
            if (_log == false)
                return;
            StringBuilder sb = new StringBuilder();
            unsafe
            {
                ...
                {
                    sb.Append(x[i].ToString().Replace(",", ".") + ", " +
                              y[i].ToString().Replace(",", ".") + "\r\n");
                }
            }
            Log(sb.ToString());
        }

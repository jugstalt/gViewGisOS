...
namespace gView.Framework.SpatialAlgorithms
{
    public class Clip
    {
        ...
        private static IPolyline ClipPolyline2Envelope_old(IEnvelope envelope, IPolyline line)
        {
            ...
            for (int pathIndex = 0; pathIndex < line.PathCount; pathIndex++)
            {
                ...
                IPoint point = path[0];
                double[] p0 = { point.X, point.Y };
                int code0 = CalculateLRBT(envelope, point);

                Path newPath = null;
                for (int i = 1; i < path.PointCount; i++)
                {
                    point = path[i];
                    double[] p1 = { point.X, point.Y };
                    int code1 = CalculateLRBT(envelope, path[i]);
                    ...
                    if (code0==0 && code1==0)
                    {
                        ...
                        {
                            newPath = new Path();
                            newPath.AddPoint(new Point(p0[0], p0[1]));
                        }
                        newPath.AddPoint(new Point(p1[0], p1[1]));
                        p0[0] = p1[0];
                        p0[1] = p1[1];
                        code1 = code0;
                        continue;
                    ...
                    double[] pdiff ={ p1[0] - p0[0], p1[1] - p0[1] };
                    ...
                    for (int edge = 0; edge < 4; edge++)
                    {
                        ...
                        {
                            case 0:
                                if (pdiff[0] == 0.0) continue;
                                y = p0[1] + pdiff[1] / pdiff[0] * (1.0 + envelope.minx);
                                if (y < envelope.miny || y > envelope.maxy) continue;

                                N[0] = -1.0; N[1] = 0.0;
                                pe[0] = envelope.minx; pe[1] = 0.0;
                                break;
                            case 1:
                                if (pdiff[1] == 0.0) continue;
                                x = p0[0] + pdiff[0] / pdiff[1] * (1.0 + envelope.miny);
                                if (x < envelope.minx || x > envelope.maxx) continue;
                            ...
                            case 2:
                                if (pdiff[0] == 0.0) continue;
                                y = p0[1] + pdiff[1] / pdiff[0] * (1.0 + envelope.maxx);
                                if (y < envelope.miny || y > envelope.maxy) continue;
                            ...
                            case 3:
                                if (pdiff[1] == 0.0) continue;
                                x = p0[0] + pdiff[0] / pdiff[1] * (1.0 + envelope.maxy);
                                if (x < envelope.minx || x > envelope.maxx) continue;
                    ...
                    if (tpe != 1e10 || tpl != 1e10)
                    {
                        ...
                        if (tpe_ < tpl_)
                        {
                            double[] pE = { p0[0] + tpe_ * pdiff[0], p0[1] + tpe_ * pdiff[1] };
                            double[] pL = { p0[0] + tpl_ * pdiff[0], p0[1] + tpl_ * pdiff[1] };

                            if (newPath == null)
                            {
                                newPath = new Path();
                                newPath.AddPoint(new Point(pE[0], pE[1]));
                            }
                            newPath.AddPoint(new Point(pL[0], pL[1]));
                            if (tpl_ < 1.0) // Leaving 
                            {
                                newLine.AddPath(newPath);
                                newPath = null;
                ...
                if (newPath != null) newLine.AddPath(newPath);
                newPath = null;
            }

...
namespace gView.Framework.SpatialAlgorithms
{
    public class Algorithm
    {
        ...
        */
        ...
        #region Tristrip
        public static List<System.Drawing.Drawing2D.GraphicsPath> Convert2Tristrip(System.Drawing.Drawing2D.GraphicsPath path)
        {
            List<System.Drawing.Drawing2D.GraphicsPath> _paths = new List<System.Drawing.Drawing2D.GraphicsPath>();

            GeomPolygon poly = new GeomPolygon(path);
            GeomTristrip tristrip = poly.ToTristrip();

            for (int i = 0; i < tristrip.NofStrips; i++)
            {
                _paths.Add(tristrip.Strip[i].TristripToGraphicsPath());
            }

            return _paths;
        }
        public static List<IPolygon> PolygonTriangeles(IPolygon polygon)
        {
            GeomTristrip strip = ClipWrapper.PolygonToTristip(polygon);
            List<IPolygon> triangles = new List<IPolygon>();
            for (int i = 0; i < strip.NofStrips; i++)
                strip.Strip[i].TristripToPolyons(ref triangles);

            return triangles;
        }
        internal static List<GeomTriangle> Triangles(IPolygon polygon)
        {
            GeomTristrip strip = ClipWrapper.PolygonToTristip(polygon);
            List<GeomTriangle> triangles = new List<GeomTriangle>();
            for (int i = 0; i < strip.NofStrips; i++)
                strip.Strip[i].TripstripToTriangles(ref triangles);
        ...
        {
            return ClipWrapper.Union(polygons);
            ...
            //        polygon.AddRing(p[i]);
            //}
            //List<IPolygon> polylist = new List<IPolygon>();
            //polylist.Add(polygon);
            //return ClipWrapper.Union(polylist);
        }
        ...
        public static IPolygon FastMergePolygon(List<IPolygon> polygons, ICancelTracker cancelTracker, gView.Framework.UI.ProgressReporterEvent reporter)
        {
            ...
            for (int i = 0; i < polygons.Count; i += 2)
            {
                ...
                if (i + 1 < count)
                {
                    ...
                    p.Add(polygons[i]);
                    p.Add(polygons[i + 1]);
                    merged.Add(ClipWrapper.Union(p));
                }
                else
                {
        ...
        internal static IPolygon PolylineBuffer(IPolyline polyline, double distance)
        {
            ...
            foreach (IPolygon buffer in buffers)
            {
                if (geomPolygon == null)
                {
                    geomPolygon = new GeomPolygon(buffer);
                }
                else
                {
                    GeomPolygon b = new GeomPolygon(buffer);
                    geomPolygon = geomPolygon.Clip(ClipOperation.Union, b);
                }
            }
            return (geomPolygon == null) ? null : geomPolygon.ToPolygon();
             * */
        }
        private static void PathBuffers(IPath path, double distance, ref List<IPolygon> polygons)
        {
            ...
            /*
            List<SimpleLineSegment> lines = CreateLineSegments(points);

            foreach (SimpleLineSegment line in lines)
            {
                IPolygon buffer = line.Buffer(distance);

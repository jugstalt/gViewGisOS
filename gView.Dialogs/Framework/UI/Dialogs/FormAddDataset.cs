		...
		private void ButtonDatasetOK_Click(object sender, System.EventArgs e)
		{
            ...
            while ((dataset = _datasets.Next) != null)
            {
                ...
                if (dataset is IFeatureDataset && _map is Map && _map is IDisplay)
                {
                    ...
                    mapLimit.minx = Math.Min(mapLimit.minx, fDS.Envelope.minx);
                    mapLimit.miny = Math.Min(mapLimit.miny, fDS.Envelope.miny);
                    mapLimit.maxx = Math.Max(mapLimit.maxx, fDS.Envelope.maxx);
                    mapLimit.maxy = Math.Max(mapLimit.maxy, fDS.Envelope.maxy);
                    ((IDisplay)_map).Limit = mapLimit;

                    if (first)
                    {
                        minx = fDS.Envelope.minx;
                        miny = fDS.Envelope.miny;
                        maxx = fDS.Envelope.maxx;
                        maxy = fDS.Envelope.maxy;
                        first = false;
                    }
                    else
                    {
                        minx = Math.Min(fDS.Envelope.minx, minx);
                        miny = Math.Min(fDS.Envelope.miny, miny);
                        maxx = Math.Max(fDS.Envelope.maxx, maxx);
                        maxy = Math.Max(fDS.Envelope.maxy, maxy);
                    }
                }
